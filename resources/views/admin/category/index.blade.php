                ...
                <div class="card shadow-sm overflow-hidden" style="border: 1px solid #cbd5e1 !important; border-radius: 1rem !important; background: #ffffff;">
                    ...
                    <div class="card-header border-0 py-3 px-4 d-flex flex-wrap justify-content-between align-items-center gap-2" style="background: linear-gradient(135deg, #1e293b 0%, #0f172a 100%); color: white;">
                        <div class="d-flex align-items-center gap-3">
                            <h4 class="mb-0 fw-bold text-white tracking-tight" style="font-size: 1.25rem;">ADMINISTRACIÓN DE CATEGORÍAS</h4>
                            <span class="badge bg-success rounded-pill px-3 py-1 font-weight-bold" style="font-size: 0.75rem; letter-spacing: 0.5px;">CONFIGURACIÓN</span>
                        </div>
                        <div>
                            <a class="btn rounded-pill px-4 shadow-sm font-weight-bold text-white" href="{{ url('admin/categories/create') }}" role="button" style="background-color: #2563eb; border-color: #2563eb;">
                                <i class="fa fa-plus me-1"></i> {{ trans('admin.category.actions.create') }}
                            </a>
                        </div>
                    </div>
                    ...
                    <div class="card-body p-4" v-cloak>
                            ...
                            <table class="table table-hover table-listing mb-0 align-middle">
                                <thead class="bg-light">
                                    <tr>
                                        ...
                                        <th is='sortable' :column="'id'" class="text-dark font-weight-bold" width="100px">{{ trans('admin.category.columns.id') }}</th>
                                        <th is='sortable' :column="'name'" class="text-dark font-weight-bold">{{ trans('admin.category.columns.name') }}</th>
                                    ...
                                    <tr v-show="(clickedBulkItemsCount > 0) || isClickedAll">
                                        <td class="bg-bulk-info d-table-cell text-center" colspan="4">
                                            ...
                                            <span class="pull-right pr-2">
                                                <button class="btn btn-sm btn-danger rounded-pill px-3" @click="bulkDelete('/admin/categories/bulk-destroy')">{{ trans('brackets/admin-ui::admin.btn.delete') }}</button>
                                            </span>
                                        </td>
                                    </tr>
                                </thead>
                        ...
                        <div class="no-items-found text-center py-5" v-if="!collection.length > 0">
                            ...
                            <a class="btn rounded-pill px-4 shadow-sm font-weight-bold text-white mt-2" href="{{ url('admin/categories/create') }}" role="button" style="background-color: #2563eb; border-color: #2563eb;">
                                <i class="fa fa-plus me-1"></i> {{ trans('admin.category.actions.create') }}
                            </a>
                        </div>
                    </div>
