                ...
                <div class="card shadow-sm overflow-hidden" style="border: 1px solid #cbd5e1 !important; border-radius: 1rem !important; background: #ffffff;">
                    ...
                    <div class="card-header border-0 py-3 px-4 d-flex flex-wrap justify-content-between align-items-center gap-2" style="background: linear-gradient(135deg, #1e293b 0%, #0f172a 100%); color: white;">
                        <div class="d-flex align-items-center gap-3">
                            <h4 class="mb-0 fw-bold text-white tracking-tight" style="font-size: 1.25rem;">ASISTENCIAS FINALIZADAS</h4>
                            <span class="badge bg-primary rounded-pill px-3 py-1 font-weight-bold" style="font-size: 0.75rem; letter-spacing: 0.5px;">FINALIZADO</span>
                        </div>
                    </div>
                    ...
                    <div class="card-body p-4" v-cloak>
                        ...
                        <!-- Tabla de Asistencias Finalizadas -->
                                ...
                                <tbody>
                                    <tr v-for="(item, index) in collection" :key="item.id" :class="bulkItems[item.id] ? 'bg-bulk' : ''">
                                        ...
                                        <td class="text-center">
                                            <span class="badge rounded-pill bg-primary px-3 py-1 font-weight-bold">@{{ item.statuses && item.statuses.state ? item.statuses.state.name : 'FINALIZADO' }}</span>
                                        </td>
                                    ...
                                    <tr v-if="!collection || collection.length === 0">
                                        <td colspan="10" class="text-center py-5 text-muted">
                                            <i class="fa fa-check-circle-o fa-3x mb-3 text-success opacity-75 d-block"></i>
                                            <h6 class="fw-bold mb-1">No se encontraron asistencias finalizadas</h6>
                                            <p class="small text-muted mb-0">Los registros de asistencias resueltas aparecerán aquí.</p>
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
